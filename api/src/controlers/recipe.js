...
const createRecipe = async ({
  ...
  let cuisinesFound = await Cuisine.findAll({
    where: {
      cuisine: cuisines,
    },
  });
  newRecipe.addCuisine(cuisinesFound);

  let dishTypesFound = await DishType.findAll({
    where: {
      dishType: dishTypes,
    },
  });
  newRecipe.addDishType(dishTypesFound);
